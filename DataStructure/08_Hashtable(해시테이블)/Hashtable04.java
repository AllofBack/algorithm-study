...
class MyHashTable4 extends MyHashTable {
    ...
    public int getHashC(int size) {
        ...
        for (int i = size - 1; i > 2; i--) {
            boolean isPrime = true; // 소수인지 아닌지 판별
            for (int j = 2; j < i; j++) {
                if (i % j == 0) { // 소수인지 확인하는 절차
                    isPrime = false;
                    break;
                }
            }
    ...
    public int getHash2(int key) { // 2번째 해시방법
    ...
    public void setValue(int key, int data) {
        ...
        if (this.elemCnt == this.table.length) {
            ...
            int cnt = 1;
            while (true) {
                newIdx = (newIdx + this.getHash2(newIdx) * cnt) % this.table.length; // hash2 사용
