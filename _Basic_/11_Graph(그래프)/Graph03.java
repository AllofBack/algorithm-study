...
class MyGraphMatrix2 extends MyGraphMatrix {
    ...
    public void dfs(int id) {
        boolean[] visited = new boolean[this.elemCnt]; // 방문배열 확인해주기
        ...
        while (!stack.isEmpty()) {
            int curId = stack.pop(); // 꺼내서 출력하기
            System.out.print(this.vertices[curId] + " ");

            for (int i = this.elemCnt - 1; i >= 0; i--) { // 인접 행렬 정보를 통해 확인(역순 탐색)
                if (this.adjMat[curId][i] == 1 && visited[i] == false) { // 1이면 간선정보가 있는 경우.
                    stack.push(i);
                    visited[i] = true;
                }
            }
